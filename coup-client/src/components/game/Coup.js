...
export default class Coup extends Component {
    ...
    render() {
        ...
        if (this.state.currentPlayer) {
            currentPlayer = <h3>It is <b>{this.state.currentPlayer}</b>'s turn</h3>
        }
        ...
        if (this.state.playerIndex != null && !this.state.isDead) {
            influences = <>
                ...
                {this.state.players[this.state.playerIndex].influences.map((influence, index) => {
                    return <div key={index} className="InfluenceUnitContainer">
                        <img src={`${this.influenceCard[influence]}`} alt={influence} />
                        <br></br>
                        <h3>{influence}</h3>
                    </div>
                })
            ...
            coins = this.state.players[this.state.playerIndex].money;


            for (var i = 0; i < this.state.players[this.state.playerIndex].money; i++) {
                getCoins.push(<img src={Coin} alt={this.state.players[this.state.playerIndex].money} />)
            }
        ...
        return (
                <div className="GameContainer">
                    ...
                    <PlayerBoard players={this.state.players} current={this.props.name} ></PlayerBoard>
                    <hr></hr>
                    <div className="GameHeader">
                        <RulesModal />
                        <CheatSheetModal />
